...
- 누구나 **이름 + 비밀번호(6자 이상)** 로 들어간다. 처음 보는 이름이면 그 자리에서 계정이 만들어진다. 로그인 탭과 가입 탭은 없고 한 폼에서 둘 다 처리한다.
...
### 2.1 로그인 (가입 겸용)
- 입력: 이름, 비밀번호(6자 이상). 버튼 하나(`들어가기`). 탭 없음.
- 이름은 공백 제거 후 1~20자, 고유.
- 동작: 먼저 로그인을 시도하고, 자격 증명 오류면 그 이름으로 가입해 바로 로그인된다. 이름은 있는데 비밀번호가 틀리면 "이미 있는 이름입니다. 비밀번호를 다시 확인하세요."
- 내부적으로 이름을 `u-<utf8 hex>@board.local` 가짜 이메일로 바꿔 Supabase Auth에 넣는다. 이메일 확인은 프로젝트 설정에서 끈다.
- 비밀번호 분실 시 복구 수단은 없다. 관리자가 Supabase 대시보드에서 계정을 지우고 다시 만들게 한다.
