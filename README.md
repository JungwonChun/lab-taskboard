...
> **다른 NAS·서버에서 직접 띄우려면 → [SELF_HOST.md](SELF_HOST.md)**
> 빌드 과정이 없는 정적 페이지라, 파일만 내려주는 웹서버면 어디서든 돕니다.
...
- **로그인은 이름 + 비밀번호**입니다. 처음 보는 이름이면 그 자리에서 계정이 만들어집니다.
...
- 비밀번호 분실 시 복구 수단은 없습니다. 관리자가 Supabase → Authentication → Users 에서 해당 계정을 지우면, 같은 이름으로 새로 만들 수 있습니다.
